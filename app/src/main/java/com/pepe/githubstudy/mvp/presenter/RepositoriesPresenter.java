...
import com.pepe.githubstudy.dao.BookMarkRepo;
import com.pepe.githubstudy.dao.BookMarkRepoDao;
import com.pepe.githubstudy.dao.DaoSession;
import com.pepe.githubstudy.dao.TraceRepo;
import com.pepe.githubstudy.dao.TraceRepoDao;
...
//    @Subscribe
    ...
    private void loadTrace(int page) {
        ...
        LogUtil.d("loadTrace = " + (System.currentTimeMillis() - start));
        ...
        Observable.just(page)
                ...
                .subscribe(new Consumer<Object>() {
                    ...
                    public void accept(Object results) throws Exception {
                        ...
                        if (results != null) {
                            repos = (ArrayList<Repository>) results;
                            mView.hideLoading();
                            mView.showRepositories(repos);
                        } else {
...
//        String ownerAvatar = element.select("div > div > a > img").attr("src");
    ...
    private void loadTrending(boolean isReload) {
        ...
        generalRxHttpExecute(new IObservableCreator<ResponseBody>() {
            @Override
            public Observable<Response<ResponseBody>> createObservable(boolean forceNetWork) {
                return getGitHubWebPageService().getTrendingRepos(forceNetWork, language.getSlug(), since.name());
            }
        }, httpObserver, !isReload);
    }
