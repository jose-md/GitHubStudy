...
import com.pepe.githubstudy.http.core.HttpProgressSubscriber;
import com.pepe.githubstudy.http.core.HttpResponse;
import com.pepe.githubstudy.mvp.contract.IRepositoryContract;
...
public class RepositoryPresenter extends BasePresenter<IRepositoryContract.View>
    ...
    public void loadBranchesAndTags() {
        ...
        Observable<Response<ArrayList<Branch>>> observable = getRepoService().getBranches(owner, repoName)
                .flatMap(new Function<Response<ArrayList<Branch>>, ObservableSource<Response<ArrayList<Branch>>>>() {
                    @Override
                    public ObservableSource<Response<ArrayList<Branch>>> apply(Response<ArrayList<Branch>> arrayListResponse) throws Exception {
                        branches = arrayListResponse.body();
                        return getRepoService().getTags(owner, repoName);
                    }
                });
        generalRxHttpExecute(observable, httpProgressSubscriber, null);
    }
